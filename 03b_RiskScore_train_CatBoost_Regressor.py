...
- We now train a CatBoostRegressor to predict a continuous "risk score".
- The target is still derived from your existing binary column, but treated as numeric 0/1.
- For evaluation (and to keep all the same classification-style artifacts), we apply a translator:
    risk_score >= 0.5  -> predicted class 1 ("Bug")
    risk_score <  0.5  -> predicted class 0 ("No Bug")

Folder + artifact labeling:
- All model folders and outputs are written under a *_riskscore suffix.
  This means:
...
# NEW: suffix to label risk-score regressor artifacts
MODEL_SUFFIX = "_riskscore"

OUT_DIR = Path(f"data_{TARGET}") / f"models_{str(RANDOM_STATE)}{MODEL_SUFFIX}"
...
BAYES_CV = 3
# NEW: regression-friendly scoring (only relevant if SKIP_BAYES_OPTIMIZATION=False)
BAYES_SCORING = "neg_mean_squared_error"
BAYES_N_JOBS = 1
...
# NEW: translator threshold for risk score -> class label
...
def ensure_binary_target(y: pd.Series) -> pd.Series:
    """
    Ensures the target is numeric 0/1 (stored as int-like).
    Plain meaning:
    - We keep the same underlying meaning, but treat it as numeric for the regressor.
    """
    if y.dtype == bool:
        return y.astype(int)
    if y.dtype == object:
...
def risk_to_class(risk_scores: np.ndarray, threshold: float = RISK_SCORE_THRESHOLD) -> np.ndarray:
    """
    Translator: continuous risk score -> hard class.
      >= threshold -> 1
      <  threshold -> 0
    """
...
def metrics_binary(y_true: np.ndarray, y_pred: np.ndarray, y_score: Optional[np.ndarray]) -> Dict[str, float]:
    out: Dict[str, float] = {}
...
def eval_on(dfX: pd.DataFrame, dfy: pd.Series, model: CatBoostRegressor) -> Tuple[Dict[str, float], np.ndarray]:
    """
    Evaluate regressor as a classifier by translating risk score -> class.
    """
    risk = model.predict(dfX)
    pred = risk_to_class(np.asarray(risk), threshold=RISK_SCORE_THRESHOLD)
    m = metrics_binary(dfy.to_numpy(), pred, np.asarray(risk))
...
def build_model(params: Dict, seed: int, cat_feature_indices: List[int]) -> CatBoostRegressor:
...
def build_ootb_model(seed: int, cat_feature_indices: List[int]) -> CatBoostRegressor:
    return CatBoostRegressor(
        #task_type="GPU", #GPU
        #devices=CATBOOST_GPU_DEVICE,
        allow_writing_files=False,
...
def repeated_eval_balanced(X: pd.DataFrame, y: pd.Series, model: CatBoostRegressor, seed0: int) -> Dict:
    """
...
def main() -> None:
    ...
    # Target remains 0/1, but the model is a regressor (risk score)
    ...
    best_overall_model: Optional[CatBoostRegressor] = None
    ...
    for split_i in range(N_REPEATS):
        ...
        if SKIP_BAYES_OPTIMIZATION:
            ...
            base_for_bayes = CatBoostRegressor(
                #task_type="GPU",
                #devices=CATBOOST_GPU_DEVICE,
                allow_writing_files=False,
                verbose=VERBOSE,
    ...
    # Label features.json as risk score model, but keep target name consistent for downstream loading.
    (OUT_DIR / "features.json").write_text(
        json.dumps({"target": TARGET, "features": feats, "model_type": "riskscore_regressor"}, indent=2),
        encoding="utf-8",
    )
    ...
    training_report = {
        "target": TARGET,
        "model_type": "riskscore_regressor",
        "risk_score_threshold": RISK_SCORE_THRESHOLD,
        "features": feats,
    ...
    report_lines.append(f"MODEL_TYPE: riskscore_regressor")
    report_lines.append(f"RISK_SCORE_THRESHOLD: {RISK_SCORE_THRESHOLD}")
    ...
    report_lines.append("  - Trained CatBoostRegressor to output a continuous risk score.")
    report_lines.append("    This means the model output is not a hard class label.")
    report_lines.append(f"  - Translated risk score to class via threshold {RISK_SCORE_THRESHOLD}.")
    report_lines.append("    This means all reported classification metrics come from that translator.")
