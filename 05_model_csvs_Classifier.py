...
Classifier workflow (CatBoostClassifier).

Same file structure/logic as your risk-score batch script, but adapted back to the
regular classifier and with the “risk score” relabel done correctly:
...
- Append TWO prediction columns:
    {TARGET}_predicted
    {TARGET}_risk_score   (this is just predicted_proba for class 1, clipped to [0,1])
...
# Threshold for converting probability -> class
THRESHOLD = 0.5
...
def clip_unit_interval(x: np.ndarray) -> np.ndarray:
...
def balanced_eval_100x(
    ...
    for i in range(n_iterations):
        ...
        try:
            proba_under = model.predict_proba(X_under)[:, 1]
            proba_under = clip_unit_interval(proba_under)
            y_pred_under = (proba_under >= THRESHOLD).astype(int)
        except Exception:
            y_pred_under = model.predict(X_under).astype(int)
...
def process_one_csv(
    ...
    try:
        ...
        proba = clip_unit_interval(proba)
    ...
    # “Risk score” relabel for classifier: risk_score := predicted_proba(class=1)
    risk_score_col = f"{TARGET}_risk_score"
    if has_proba:
        df[risk_score_col] = proba.astype(float)

    # Reorder at end: true label, predicted class, risk score (side-by-side)
    df = reorder_end_columns(df, [TARGET, pred_class_col, risk_score_col])

    out_csv = input_path.with_name(input_path.stem + "_predicted_Classifier.csv")
    ...
    report_txt.append(f"MODEL_TYPE: CatBoostClassifier")
    report_txt.append(f"TARGET: {TARGET}")
    report_txt.append(f"RANDOM_STATE: {RANDOM_STATE}")
    report_txt.append(f"THRESHOLD: {THRESHOLD}")
    if has_proba:
        report_txt.append("RISK_SCORE_COLUMN: predicted_proba(class=1) clipped to [0,1]")
    else:
        report_txt.append("RISK_SCORE_COLUMN: not written (model has no predict_proba)")
